...
https://developers.dropbox.com/oauth-guide
  ...
  static async showDropboxPicker() {
    try {
      const script = document.createElement('script');
          ...
          window.Dropbox.choose({
            success: function(files) {
              if (files && files.length > 0) {
                const file = files[0];
                CloudStorageService.downloadDropboxFile(file.link, file.name)
                  .then(resolve)
                  .catch(reject);
              } else {
                resolve(null);
  ...
  static async downloadDropboxFile(fileUrl, fileName) {
    try {
